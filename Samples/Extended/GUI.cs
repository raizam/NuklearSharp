...
namespace Extended
{
	public unsafe static class GUI
	{
		private static readonly string[] items = new[] { "Item 0", "item 1", "item 2" };
		private static readonly float[] ratio = new float[] { 0.15f, 0.85f };
		private static readonly float[] ratio2 = new float[] { 0.15f, 0.50f, 0.35f };
		private static readonly string[] items2 = new[] { "Item 0", "item 1", "item 2" };

		private static string[] text = new string[3];
		private static int[] text_len = new int[3];
		...
		private static int selected_item = 0;
		private static int selected_image = 3;
		private static int selected_icon = 0;
		private static bool piemenu_active = false;
		...
		{
			int ret = -1;
			Nuklear.nk_rect total_space;
			Nuklear.nk_rect bounds;
			int active_item = 0;

			/* pie menu popup */
			Nuklear.nk_color border = ctx.Ctx.style.window.border_color;
			Nuklear.nk_style_item background = ctx.Ctx.style.window.fixed_background;
			...
			if (ctx.PopupBegin (Nuklear.NK_POPUP_STATIC, "piemenu", Nuklear.NK_WINDOW_NO_SCROLLBAR,
				    Nuklear.nk_rect_ (pos.x - total_space.x - radius, pos.y - radius - total_space.y,
					    2 * radius, 2 * radius))) {
				int i = 0;
				Nuklear.nk_command_buffer o = ctx.WindowGetCanvas ();
				Nuklear.nk_input inp = ctx.Ctx.input;
				...
				{
					/* circle buttons */
					float step = (2 * 3.141592654f) / (float)(Math.Max (1, item_count));
					float a_min = 0;
					float a_max = step;

					Nuklear.nk_vec2 center = Nuklear.nk_vec2_ (bounds.x + bounds.w / 2.0f, bounds.y + bounds.h / 2.0f);
					Nuklear.nk_vec2 drag = Nuklear.nk_vec2_ (inp.mouse.pos.x - center.x, inp.mouse.pos.y - center.y);
					float angle = (float)Math.Atan2 (drag.y, drag.x);
					if (angle < -0.0f)
					...
					for (i = 0; i < item_count; ++i) {
						...
						rx = bounds.w / 2.0f;
						ry = 0;
						dx = rx * (float)Math.Cos (a_min) - ry * (float)Math.Sin (a_min);
						dy = rx * (float)Math.Sin (a_min) + ry * (float)Math.Cos (a_min);
		...
		public static void grid_demo (ContextWrapper ctx, Media media)
		{
			...
			if (ctx.Begin ("Grid Demo", Nuklear.nk_rect_ (600, 350, 275, 250),
				...
				if (ctx.ComboBeginLabel (items [selected_item], Nuklear.nk_vec2_ (ctx.WidgetWidth (), 200))) {
     ...
     *------------------------------------------------*/
			...
			ui_widget (ctx, media, 35);
			if (ctx.ButtonSymbolLabel ((option == 0) ? Nuklear.NK_SYMBOL_CIRCLE_OUTLINE : Nuklear.NK_SYMBOL_CIRCLE_SOLID, "Select", Nuklear.NK_TEXT_LEFT))
				option = 0;
			ui_widget (ctx, media, 35);
			if (ctx.ButtonSymbolLabel ((option == 1) ? Nuklear.NK_SYMBOL_CIRCLE_OUTLINE : Nuklear.NK_SYMBOL_CIRCLE_SOLID, "Select", Nuklear.NK_TEXT_LEFT))
				option = 1;
			ui_widget (ctx, media, 35);
			if (ctx.ButtonSymbolLabel ((option == 2) ? Nuklear.NK_SYMBOL_CIRCLE_OUTLINE : Nuklear.NK_SYMBOL_CIRCLE_SOLID, "Select", Nuklear.NK_TEXT_LEFT))
				option = 2;
     ...
     *------------------------------------------------*/
		...
		public static void basic_demo (ContextWrapper ctx, Media media)
		{
			...
			ui_widget (ctx, media, 40);
			if (ctx.ComboBeginImageLabel (items2 [selected_icon], media.images [selected_icon], Nuklear.nk_vec2_ (ctx.WidgetWidth (), 200))) {
				ctx.LayoutRowDynamic (35, 1);
				for (i = 0; i < 3; ++i)
			...
			if (piemenu_active) {
				int ret = ui_piemenu (ctx, piemenu_pos, 140, media.menu, 6);
				if (ret == -2)
