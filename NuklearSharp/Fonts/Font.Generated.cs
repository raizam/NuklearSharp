...
namespace NuklearSharp
{
	public unsafe partial class Font
	{
		...
		public FontGlyph* FindGlyph(char unicode)
		{
			...
			do
			{
				count = (int) (Nuklear.RangeCount(iter.range));
				for (i = (int) (0); (i) < (count); ++i)
				{
					uint f = (uint) (iter.range[(i*2) + 0]);
					uint t = (uint) (iter.range[(i*2) + 1]);
					int diff = (int) ((t - f) + 1);
					if (((unicode) >= (f)) && (unicode <= t)) return &this.glyphs[((uint) (total_glyphs) + (unicode - f))];
		...
		public void Init(float pixel_height, char fallback_codepoint, FontGlyph* glyphs, BakedFont baked_font, Handle atlas)
		{
			...
			this.handle.width = TextWidth;

			this.handle.query = QueryFontGlyph;
