		...
		public void* Alloc(ulong size)
		{
			...
			else
			{
				if ((this.num_remaining_in_head_chunk) == (0))
				{
					...
					TtHheapChunk* c = (TtHheapChunk*) (CRuntime.malloc((ulong) ((ulong) sizeof (TtHheapChunk) + size*(ulong) (count))));
					if ((c) == (null)) return null;
					c->next = this.head;
					this.head = c;
					this.num_remaining_in_head_chunk = (int) (count);
				}
				--this.num_remaining_in_head_chunk;
				return (sbyte*) (this.head) + size*(ulong) (this.num_remaining_in_head_chunk);
			}
