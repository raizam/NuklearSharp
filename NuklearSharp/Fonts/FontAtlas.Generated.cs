// Generated by Sichem at 2/7/2018 5:59:03 PM
	...
	public unsafe partial class FontAtlas
	{
		...
		public Font Add(FontConfig config)
		{
			...
			if (config.ttf_data_owned_by_atlas == 0)
			{
				...
				Nuklear.Memcopy(cfg.ttf_blob, config.ttf_blob, (ulong) (cfg.ttf_size));
		...
		public void* Bake(ref int width, ref int height, int fmt)
		{
			...
			this.config.FontBakerMemory(&tmp_size, ref this.glyph_count, (int) (this.font_num));
			tmp = CRuntime.malloc((ulong) (tmp_size));
			if (tmp == null) goto failed;
			baker = Nuklear.FontBakerz(tmp, glyph_count, font_num);
			...
			if (baker->FontBakePack(&img_size, ref width, ref height, ref this.custom, this.config, (int) (this.font_num)) == 0)
				goto failed;
			this.pixel = CRuntime.malloc((ulong) (img_size));
			if (this.pixel == null) goto failed;
			baker->FontBake(this.pixel, (int) (width), (int) (height), this.glyphs, (int) (this.glyph_count), this.config,
				(int) (this.font_num));
			fixed (byte* ptr = Nuklear.nk_custom_cursor_data)
			{
				this.custom.FontBakeCustomData(this.pixel, (int) (width), (int) (height), ptr, (int) (90), (int) (27), ('.'), ('X'));
			}
			if ((fmt) == (Nuklear.NK_FONT_ATLAS_RGBA32))
			{
				void* img_rgba = CRuntime.malloc((ulong) (width*height*4));
				if (img_rgba == null) goto failed;
				Nuklear.FontBakeConvert(img_rgba, (int) (width), (int) (height), this.pixel);
				CRuntime.free(this.pixel);
				this.pixel = img_rgba;
			}
			...
			{
				Font font = font_iter;
				FontConfig config = font.config;
				font.Init((float) (config.size), config.fallback_glyph, this.glyphs, config.font, (Handle) (Nuklear.HandlePtr(null)));
			}
			for (i = (int) (0); (i) < (Nuklear.NK_CURSOR_COUNT); ++i)
			{
				...
				cursor.img.region[0] = ((ushort) (this.custom.x + Nuklear.nk_cursor_data[i, 0].x));
				cursor.img.region[1] = ((ushort) (this.custom.y + Nuklear.nk_cursor_data[i, 0].y));
				cursor.img.region[2] = ((ushort) (Nuklear.nk_cursor_data[i, 1].x));
				cursor.img.region[3] = ((ushort) (Nuklear.nk_cursor_data[i, 1].y));
				cursor.size = (Vec2) (Nuklear.nk_cursor_data[i, 1]);
				cursor.offset = (Vec2) (Nuklear.nk_cursor_data[i, 2]);
			}
			CRuntime.free(tmp);
			return this.pixel;
			failed:
		...
		public void End(Handle texture, DrawNullTexture* _null_)
		{
			...
			if (this == null)
			{
				...
				_null_->texture = (Handle) (texture);
				_null_->uv = (Vec2) (Nuklear.Vec2z((float) (0.5f), (float) (0.5f)));
			}

			if ((_null_) != null)
			{
				_null_->texture = (Handle) (texture);
				_null_->uv.x = (float) ((this.custom.x + 0.5f)/(float) (this.tex_width));
				_null_->uv.y = (float) ((this.custom.y + 0.5f)/(float) (this.tex_height));
			}

			for (font_iter = this.fonts; font_iter != null; font_iter = font_iter.next)
			{
				font_iter.texture = (Handle) (texture);
				font_iter.handle.texture = (Handle) (texture);
			}
			for (i = (int) (0); (i) < (Nuklear.NK_CURSOR_COUNT); ++i)
			{
				this.cursors[i].img.handle = (Handle) (texture);
			}
			CRuntime.free(this.pixel);
			this.pixel = null;
		...
		public void Cleanup()
		{
			...
			{
				FontConfig iter;
				for (iter = this.config; iter != null; iter = iter.next)
				{
					FontConfig i;
		...
		public void Clear()
		{
			if ((this.config) != null)
			{
				FontConfig iter;
				FontConfig next;
				for (iter = this.config; iter != null; iter = next)
				{
					FontConfig i;
					FontConfig n;
			...
			if ((this.fonts) != null)
			{
				Font iter;
				Font next;
				for (iter = this.fonts; iter != null; iter = next)
				{
					next = iter.next;
				}
				this.fonts = null;
