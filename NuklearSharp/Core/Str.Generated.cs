...
namespace NuklearSharp
{
	public unsafe partial class Str
	{
		...
		public int AppendTextChar(char* str, int len)
		{
			...
			mem =
				(sbyte*) (this.buffer.Alloc((int) (Nuklear.NK_BUFFER_FRONT), (ulong) ((ulong) (len)*sizeof (char)), (ulong) (0)));
		...
		public int AppendTextUtf8(char* text, int len)
		{
			...
			for (i = (int) (0); (i) < (len); ++i)
			{
				byte_len += (int) (Nuklear.UtfDecode(text + byte_len, &unicode, (int) (4)));
			}
		...
		public int AppendStrUtf8(char* text)
		{
			...
			{
				glyph_len = (int) (Nuklear.UtfDecode(text + byte_len, &unicode, (int) (4)));
		...
		public int InsertAtChar(int pos, char* str, int len)
		{
			...
			if (((this.buffer.allocated + (ulong) (len)) >= (this.buffer.memory.size)) &&
			    ((this.buffer.type) == (Nuklear.NK_BUFFER_FIXED))) return (int) (0);
			copylen = (int) ((int) (this.buffer.allocated) - pos);
			if (copylen == 0)
			{
				AppendTextChar(str, (int) (len));
				return (int) (1);
			}
		...
		public int InsertTextRunes(int pos, char* runes, int len)
		{
			...
			for (i = (int) (0); (i) < (len); ++i)
			{
				byte_len = (int) (Nuklear.UtfEncode(runes[i], glyph, (int) (4)));
				if (byte_len == 0) break;
				InsertAtRune((int) (pos + i), glyph, (int) (byte_len));
			}
			return (int) (len);
		}
		...
		{
			if ((((len == 0)) || (((ulong) (pos)) > (this.buffer.allocated))) ||
			    (((ulong) (pos + len)) > (this.buffer.allocated))) return;
			if (((ulong) (pos + len)) < (this.buffer.allocated))
			{
				sbyte* dst = ((sbyte*) ((void*) ((byte*) (this.buffer.memory.ptr) + (pos))));
				sbyte* src = ((sbyte*) ((void*) ((byte*) (this.buffer.memory.ptr) + (pos + len))));
				Nuklear.Memcopy(dst, src, (ulong) (this.buffer.allocated - (ulong) (pos + len)));
				this.buffer.allocated -= ((ulong) (len));
			}
		...
		public void DeleteRunes(int pos, int len)
		{
			...
			if ((this.len) < (pos + len))
				len =
					(int)
						(((this.len - pos) < (this.len) ? (this.len - pos) : (this.len)) < (0)
							? (0)
							: ((this.len - pos) < (this.len) ? (this.len - pos) : (this.len)));
			if (len == 0) return;
		...
		public char* AtRune(int pos, char* unicode, ref int len)
		{
			...
			while ((glyph_len) != 0)
			{
				...
				glyph_len = (int) (Nuklear.UtfDecode(text + src_len, unicode, (int) (text_len - src_len)));
			}
