	...
	unsafe partial class Context
	{
		...
		public void Build()
		{
			...
			if ((style.cursor_active != null) && (input.mouse.grabbed == 0) && ((style.cursor_visible) != 0))
			{
				...
				overlay.CommandBuffer_init(Nuklear.NK_CLIPPING_OFF);
				StartBuffer(overlay);
		...
		public int InitFixed(void* memory, ulong size, UserFont font)
		{
			...
			Nuklear.Buffer_init_fixed(this.memory, memory, size);
			return 1;
		}
		...
		public int Init(UserFont font)
		{
			...
			Buffer_init(this.memory, 4*1024);
			return 1;
		}
		...
		public int PopupBegin(int type, string title, uint flags, Rect rect)
		{
			fixed (char* ptr = title)
			{
				return PopupBegin(type, ptr, flags, rect);
			}
		}

		public static int InitDefault(UserFont font)
		{
			return Init(font);
		}
