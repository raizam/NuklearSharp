// Generated by Sichem at 2/7/2018 6:16:10 PM
...
namespace NuklearSharp
{
	...
	public unsafe partial struct RpContext
	{
		...
		{
			if ((allow_out_of_mem) != 0) this.align = (int) (1);
			else
			{
		...
		public static void RpInitTarget(RpContext* context, int width, int height, RpNode* nodes, int num_nodes)
		{
			int i;
			...
			nodes[i].next = null;
			context->init_mode = (int) (Nuklear.NK_RP__INIT_skyline);
			context->heuristic = (int) (Nuklear.NK_RP_HEURISTIC_Skyline_default);
			context->free_head = &nodes[0];
			context->active_head = &context->extra_0;
			context->width = (int) (width);
			context->height = (int) (height);
			context->num_nodes = (int) (num_nodes);
			context->RpSetupAllowOutOfMem((int) (0));
		...
		public static RpFindresult RpSkylineFindBestPos(RpContext* c, int width, int height)
		{
			...
			while (node->x + width <= c->width)
			{
				int y;
				int waste;
				y = (int) (c->RpSkylineFindMinY(node, (int) (node->x), (int) (width), &waste));
				if ((c->heuristic) == (Nuklear.NK_RP_HEURISTIC_Skyline_BL_sortHeight))
				{
					if ((y) < (best_y))
					{
						best_y = (int) (y);
			...
			if ((c->heuristic) == (Nuklear.NK_RP_HEURISTIC_Skyline_BF_sortHeight))
			{
				tail = c->active_head;
				node = c->active_head;
				prev = &c->active_head;
				...
				while ((tail) != null)
				{
					...
					y = (int) (c->RpSkylineFindMinY(node, (int) (xpos), (int) (width), &waste));
					if ((y + height) < (c->height))
					{
						if (y <= best_y)
						{
		...
		public static RpFindresult RpSkylinePackRectangle(RpContext* context, int width, int height)
		{
			RpFindresult res = (RpFindresult) (RpSkylineFindBestPos(context, (int) (width), (int) (height)));
			RpNode* node;
			RpNode* cur;
		...
		public static void RpPackRects(RpContext* context, RpRect* rects, int num_rects)
		{
			int i;
			...
			RpRect.RpQsort(rects, (uint) (num_rects), Nuklear.RectHeightCompare);
			for (i = (int) (0); (i) < (num_rects); ++i)
			{
				RpFindresult fr = (RpFindresult) (RpSkylinePackRectangle(context, (int) (rects[i].w), (int) (rects[i].h)));
			...
			RpRect.RpQsort(rects, (uint) (num_rects), Nuklear.RectOriginalOrder);
			for (i = (int) (0); (i) < (num_rects); ++i)
			{
